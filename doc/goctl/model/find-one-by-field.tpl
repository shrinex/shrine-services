func (m *default{{.upperStartCamelObject}}Model) TxFindOneBy{{.upperField}}(ctx context.Context, tx sqlx.Session, {{.in}}) (*{{.upperStartCamelObject}}, error) {
    var resp {{.upperStartCamelObject}}
	query := fmt.Sprintf("select %s from %s where {{.originalField}} limit 1", {{.lowerStartCamelObject}}Rows, m.table )
	err := tx.QueryRowCtx(ctx, &resp, query, {{.lowerStartCamelField}})
	switch err {
	case nil:
		return &resp, nil
